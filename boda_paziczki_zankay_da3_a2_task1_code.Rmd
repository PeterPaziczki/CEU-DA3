...
What we see is that both PLS and the linear regression curves are within the CI bounds of lowess, and vica versa. 
Also, the confidence intervals around the linear and lowess overlap over the entire range of data, and the same is true to the PLS and lowess relation.
These suggest that both are pretty good estimators.

We would keep the linear (level - log) model or the following reasons:
- In R square this is very close to the best models (which is PLS), the difference in very small,
- The simple linear has a big advantage: its simplicity,
- PLS shows some negative slope towards the high end oog Log GDP, something that we do not observe in the lowess.

For very similar reasons we prefer the linera level - log compared to some other models that have marginally better R-square, such as Quadratic Level -Log or Cubic Level - Log.
...
myhotels3 <- myhotels2 [, .(name, distance, stars, badge_excellence, rating, rating_reviewcount, price)] #remove not needed data
```
...
We were investiagting potential non-linearity by using lowess non-parametric regression first. This regression is supposed to uncover potential non-linearities. At first let's estimate a lowess regression on lnprice and distance.

```{r Q223_lowessGraph_distance, results='hide'}
# Estimating a lowess non-parametric regression on the data set.
...
The lowess regression shows that there is a strong negative relation between price and distance very close to the city center, but at about 1km away from the center there is a change in the relation. At approximately 1.5km the relation becomes negative again, but it is much flatter now than in the first section, at the very end it becomes totally flat. It is important to highlight that there are only a few observation in the 2-4 km range.

Then let's estimate a lowess regression on ln price and rating.

```{r Q223_lowessGraph_rating, results='hide'}

regQ2_loess_rating <- loess (lnprice ~ rating, myhotels3)
ggplot(data = myhotels3, aes(x=rating, y=lnprice)) +
  geom_point(size=1.5) +
  ggtitle ("Q223. Lowess regression") + labs(x="rating",y="lnprice") +
  geom_line(data=myhotels3,aes(x=rating,y=predict(regQ2_loess_rating)),colour="blue",size = 1)

# no chart for stars, as it is categoric
```

It shows the beyond rating 2 there is a general positive relation between lnprice and rating.
...
# below the same on log (price)

R2Table_Log <- data.table (c("log - lev lin", "log - log", "Log Quadratic", "Log Qubic"), 
                       R2s (myhotels3 [, distance], myhotels3 [, log (price)]), 
                       R2s (myhotels3 [, stars], myhotels3 [, log(price)]),
                       R2s (myhotels3 [, Gold], myhotels3 [, log(price)]),
                       R2s (myhotels3 [, Loved], myhotels3 [, log(price)]),
                       R2s (myhotels3 [, Top], myhotels3 [, log(price)]),
                       R2s (myhotels3 [, rating], myhotels3 [, log(price)])
)
names(R2Table_Log) <- c("RegType", "Distance", "Stars", "Gold", "Loved","Top", "Rating")
```

Below is a summary on the R squares of the various linear and non linear regressions. In the first table it is for price level, in the second one for log price.
```{r Q224_results_of-R2Table}
pander(R2Table)
pander (R2Table_Log)
```
As we saw in the price histogram, price is very much right skewed, we therefore analize log price.
Based on R-square we select the following functional forms:

- Distance: Cubic, because of the price - distance lowess chart we saw (has local min and max too)

- Stars: Linear, as the realtive R-square gains of anz higher order polinom would not justify the extra complexity

- Gold dummy variable: due to its poor R-square we probably ignore it, but let us see later

- Loved dummy variable: we use the linear, as this is a binary so no point using any other

- Rating: Quadratic, because of the price - rating lowess chart we saw (close to parabola)
...
We consider the above parameters for multiple regression.
```{r Q223 multiple_regression}
myhotels3 [, distance_2 := distance ^2]
myhotels3 [, distance_3 := distance ^3]
myhotels3 [, rating_2 := rating ^2]
multireg <- lm (log(price) ~ distance + distance_2 + distance_3 + stars + Top + Loved + rating + rating_2, data = myhotels3)
multireg1 <- lm (log(price) ~ distance + distance_2 + distance_3 + stars + Gold + rating + rating_2, data = myhotels3)
summary (multireg)
summary (multireg1)
```

R saquare seems to be fairly OK, much better compared to any simple regression of any shape.

Looking at the coefficients we can see that some add little and close to zero, so let us see what happens to R square if we drop those variables and simplify the model.
We drop: distance_2, distance_3, Gold, Loved.
For the rets, the multiple regression is re-calculated.

```{r Q223 multiple_regression_simplified}
multireg2 <- lm (log(price) ~ distance + stars + Top + rating + rating_2, data = myhotels3)
summary (multireg2)
```

As we see it was a good simplification: we got rid of a few "complicated" variables while kept R-square almost as high as it was.
...
Let's pick stars, based on the results we can observe that it is significant at 0.1%. The lsope means that price is x% higher for observations with one more star.

Rating is significant at 1% but not significant at 0.1%.
...
#summary(simplereg1)
#onfint(simplereg1)
#ggplot(data = myhotels3, aes(x=distance, y=lnprice)) + geom_point(size=0.5, colour="red") + geom_smooth(method="lm", colour="black")
...
#summary(simplereg2)
#confint(simplereg2)
#ggplot(data = myhotels3, aes(x=rating, y=lnprice)) + geom_point(size=0.5, colour="red") + geom_smooth(method="lm", colour="black")
...
The strategy to find the best deal is to find offers that have the largest negative residuals. The regression line tells us what would be the price on average. The greater the residual is in a negative sense, the better deal we find.
...
The 3 best deal hotels are in the below table.

```{r Q224_strategy}
myhotels3 [, res := residuals (multireg2)]
myhotels3 [,][order((residuals(multireg2)))][c(1,2,3),]
```
