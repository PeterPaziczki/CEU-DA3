...
#### Data Cleaning
As the first step of data cleaining we have downloaded a csv file containing the list of `r n_country` countries in the world as of now. There were many entries in our data set that were not about countries but some aggregations, so they were dropped.
...
ggplot(myhotels1, aes(x=distance, y=price)) + geom_point(shape = 1) + ggtitle(labs(title = "Q2. Scatterplot of price on distance", x = "Distance from the city center [km]", y = "Price [EUR]"))

# SUMMARY STATISTICS ON PRICE
qplot(myhotels1[,price], geom="histogram", binwidth=20) + theme_bw() + ggtitle(labs(title = "Q2. Histogram of price variable", x = "Price [EUR]"))
price_stat<-myhotels1[,.(mean_price = mean(price) , sd_price = sd(price), 
...
qplot(myhotels1[,distance], geom="histogram", binwidth=0.4) + theme_bw() + ggtitle(labs(title = "Q2. Histogram of distance variable", x = "Distance from the city center [km]"))

distance_stat <-myhotels1[,.(mean_dist = mean(distance) , sd_dist = sd(distance),
