...
```{r Q1_GDPSummary}
summary(Data[,GDP])
ggplot (Data, aes(GDP)) + geom_histogram ( binwidth=6) + ggtitle(labs(title = "Histogram of GDP variable", x = "GDP per capita [1000 USD]"))
...
```{r Q1_lnGDP}
summary(Data[,lnGDP])
ggplot(Data, aes(lnGDP)) + geom_histogram (binwidth=0.5) + ggtitle(labs(title = "Histogram of lnGDP variable", x = "logarithm of GDP per capita [1000 USD]"))
```
...
ggplot(Data, aes(x = lnGDP, y = life)) + geom_point ()
```
...
reg_lin <- lm(life ~ lnGDP, data=Data)
ggplot (Data, aes (x=lnGDP, y = life )) + geom_point (size = 1.5) + 
ggtitle("Q2. Linear Regression") + labs(x = "GDP per capita [1000 USD] (log)", y = "Life Expectancy [year]") + 
geom_smooth (method = "lm")
summary (reg_lin)
```
We seem to have rather small residual errors and pretty good R-square, confirming the relationship between lnGDP and Life Expectancy.
The coefficients: at 0 lnGDP the Expected Life is ~58 years, and at every percent in GDP change results in ~5.5/100 year change in average life expectancy.
...
ggplot(Data, aes(x=GDP, y=life)) +  geom_point(size=1.5) + 
  ...
  labs(x="GDP per capita, PPP [USD]", y="Life expectancy at birth [years]") +
  geom_line(data=Data,aes(x=GDP,y=predict(reg_quad)),colour="blue")
```
...
#bd Data$e2 <- resid(reg2)
highGDP <- Data[GDP>60,]

# PIECEWISE LINEAR SPLINE IN LEVEL-LOG REGRESSION
knots <- c(4)
reg3 <- lm(life ~ lspline(lnGDP,knots), data=Data)
#coeftest(reg3)
#Data$e3 <- resid(reg3)

ggplot(data = Data, aes(x=GDP, y=life)) +
  geom_point(size=1.5) +
  geom_smooth(method="lm", formula=y~bs(x, degree=1, knots), colour="navy", se=F) 

# QUADRATIC IN LEVEL-LOG REGRESSION
Data[,lnGDP_sq := lnGDP^2]
