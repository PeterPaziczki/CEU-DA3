...
```{r Q6_probit_computation, results='hide'}
# PROBIT
...
To compare the results of coefficients of both Probit and Logit models to the results of LPM regression (2) I have created the below table. In column one you can see the coefficient of regression (2) from the previous exercise. The next two columns represents the coefficients of the Logit model and Probit model, respectively. We can see that the coefficients provided by the Logit model are approximately 20 times more than the results of LPM. These values are meaningless, we cannot interpret them. The Probit modell provided the same coefficients as the LPM regression.

```{r Q6_stargazer_logit_probit}
stargazer(list(lpm2, logitcoeffs, probitcoeffs), digits=3, type="text", omit.stat=c("LL","ser","f", "aic"))
```

As I have mentioned previously that the difference between the coefficients provided by the LPM regression and the Logit model is significant. The following plot compares the prediction of these two. The magenta line is 45 degree supporting the interpretation.
...
share$fit_logit <- fitted.values(logitcoeffs)
